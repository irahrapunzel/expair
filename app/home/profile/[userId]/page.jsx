...
export default function ProfilePage() {
  ...
  const isOwnProfile = useMemo(() => {
    if (!session?.user?.username || !userId) return false;
    return session.user.username.toLowerCase() === String(userId).toLowerCase();
  }, [session, userId]);
  ...
  useEffect(() => {
    const run = async () => {
      ...
      if (!backendUrl) {
        console.warn("[profile] No backend URL configured");
        setFirstName("");
        setLastName("");
        setUsername("");
        setEmailAdd("");
  ...
  const handleSave = async () => {
    ...
    if (!configuredBackendUrl) {
      console.warn("[profile] No backend URL configured, simulating save");
      setTimeout(() => {
  ...
  // If viewing own profile, show settings page
  if (isOwnProfile) {
    return (
      <div
  ...
  // If viewing someone else's profile, show public profile view
  return (
    <div
      className={`w-[950px] mx-auto pt-10 pb-20 text-white ${inter.className}`}
    >
      <h4 className="text-[22px] font-semibold mb-10">
        {firstName || lastName ? `${firstName} ${lastName}`.trim() : username}'s Profile
      </h4>

      {/* Public profile view content would go here */}
