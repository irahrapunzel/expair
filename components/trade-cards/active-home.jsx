...
export default function ActiveTradeHome({
  ...
  return (
    <div
      className="flex flex-col w-full md:w-[455px] rounded-[20px] border-[3px] border-[#284CCC]/80 p-[20px] md:p-[25px] gap-[15px] md:gap-[20px] relative cursor-pointer transition-all duration-300 hover:scale-[1.01]"
      style={{
        background:
    ...
    >
      ...
      <div className="flex justify-between items-center w-full">
        <div className="flex items-center gap-[10px]">
          ...
          <div className="flex items-center gap-[8px]">
            ...
            <div className="flex items-center gap-2">
              {username ? (
                <Link
                  ...
                  onClick={(e) => e.stopPropagation()}
                >
                  {/* Responsive text size: small on mobile, base on desktop */}
                  <p className="text-sm md:text-base cursor-pointer">{name}</p>
                </Link>
              ) : (
            ...
            <div className="flex items-center gap-[4px]"></div>
          </div>
        </div>
        <button
      ...
      <div className="flex flex-col gap-[10px] md:gap-[15px]">
        <div className="flex justify-between items-center w-full">
          {/* Responsive text size for offering */}
          <p className="text-sm md:text-base">{offering}</p>
          <p className="text-sm md:text-base font-semibold text-[#906EFF] whitespace-nowrap">
            {totalXp} XP
          </p>
        </div>
        <div className="flex justify-end w-full">
          <p className="text-xs md:text-sm text-white/50">Due on {deadline}</p>
        </div>
      </div>
    </div>
  );
