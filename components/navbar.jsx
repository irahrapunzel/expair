  ...
  const fetchUnreadMessages = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL || "http://127.0.0.1:8000"}/messages/unread-count/`,
        {
          headers: {
            ...
            <div className="flex lg:hidden items-center gap-3 mr-1">
              <Link href="/home/messages" onClick={handleLinkClick}>
                <div className="relative cursor-pointer p-1">
                  ...
                  {unreadMsgCount > 0 && (
                    <span className="absolute -top-1 -right-2 min-w-[16px] h-[16px] px-[3px] bg-[#0038FF] text-white text-[10px] leading-[16px] rounded-full text-center flex items-center justify-center">
